...
Write-Output 'Task 6: Linking web.config to ./public'
if (Test-Path ./web.config)
{
    Copy-Item ./web.config ./public/web.config -Force | Out-Null
    Write-Output 'Task 6: Success'
}
else
{
    Write-Error 'Task 6: Cannot find we.config in ./web.config'
}
